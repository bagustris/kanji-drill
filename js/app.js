...
// `## [YYYY-MM-DD]` (or legacy `## [x.y.z]`) heading and show it. The static
// v-number in index.html is the offline/pre-fetch fallback, so a failed
// fetch just leaves that in place.
async function loadAppVersion() {
  try {
    const res = await fetch('CHANGELOG.md');
    if (!res.ok) return;
    const text = await res.text();
    const match = text.match(/^##\s*\[(\d{4}-\d{2}-\d{2}|\d+\.\d+\.\d+)\]/m);
